...
const saveApplyingFormData = (req, res) => {
  ...
  const sql = 'INSERT INTO formfields (selectedDistrict, area, body, choosingCorporation, choosingCouncil, choosingJury, khasraIntegrated, integratedKhasraNumber, office) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)';
  const values = [
    data.selectedDistrict,
    data.area,
    data.body,
    data.choosingCorporation,
    data.choosingCouncil,
    data.choosingJury,
    data.khasraIntegrated,
    data.integratedKhasraNumber,
    data.office
  ];
...
const saveApplicantData = (req, res) => {
  ...
  const sql = 'INSERT INTO applicantdata (formId, fullName, LUB, Srno, registrationDate, Hno, neighbourhoodColony, district, surveyNumber, area, village, neighbourhoodColony4, district4, developedLandName, village5, neighbourhoodColony5, district5, relinquishment, permitPurpose, mobileNumber, email, tinGstnNumber, EWS, EWSb, side, CGRResidential, CGRLand, EWSAreaResidential, EWSAreaLand, CGRAmount, clearancePWD, clearanceWRD, clearanceCSEB, clearanceCECB, clearanceNHAI, clearancePHED, clearancePMGSY, clearanceFOREST, clearanceFireNOC, clearanceGramPanchayat, clearanceNNNPTP, clearanceRevenue, clearanceRES) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
  ...
  const values = [
    ...
    data.Hno,
    data.neighbourhoodColony,
    data.district,
    data.surveyNumber,
    data.area,
    data.village,
    data.neighbourhoodColony4,
    data.district4,
    data.developedLandName,
    data.village5,
    data.neighbourhoodColony5,
    data.district5,
    data.relinquishment,
    data.permitPurpose,
    data.mobileNumber,
    ...
    data.CGRResidential,
    data.CGRLand,
    data.EWSAreaResidential,
    data.EWSAreaLand,
    data.CGRAmount,
    data.clearancePWD || null,
    data.clearanceWRD || null,
    data.clearanceCSEB || null,
    data.clearanceCECB || null,
    data.clearanceNHAI || null,
    data.clearancePHED || null,
    data.clearancePMGSY || null,
    data.clearanceFOREST || null,
    data.clearanceFireNOC || null,
    data.clearanceGramPanchayat || null,
    data.clearanceNNNPTP || null,
    data.clearanceRevenue || null,
    data.clearanceRES || null
  ];
